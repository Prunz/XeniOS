...
pointer_t<XAudioDriverPerformance> perf_ptr) {
...
dword_result_t XAudioEnableDucker_entry(dword_t unk) {
return X_ERROR_SUCCESS;
}
DECLARE_XBOXKRNL_EXPORT1(XAudioEnableDucker, kAudio, kStub);

// Returns the current ducker output level. 1.0f = no attenuation. Returning
// 0 causes the audio manager to treat all channels as fully muted.
dword_result_t XAudioGetDuckerLevel_entry(lpfloat_t level_ptr) {
if (level_ptr) {
*level_ptr = 1.0f;
}
return X_ERROR_SUCCESS;
...
// Returns the ducker threshold. 1.0f = only full-scale voice triggers
// ducking, effectively disabling it. Returning 0 means duck at any nonzero
// voice level, causing permanent silence.
dword_result_t XAudioGetDuckerThreshold_entry(lpfloat_t threshold_ptr) {
if (threshold_ptr) {
*threshold_ptr = 1.0f;
}
return X_ERROR_SUCCESS;
...
// ducking ramps in after voice exceeds the threshold.
...
// ducking is maintained after voice drops below the threshold.
...
// the fade-back-to-normal takes after hold expires.
