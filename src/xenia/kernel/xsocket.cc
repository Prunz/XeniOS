...
X_STATUS XSocket::IOControl(uint32_t cmd, uint8_t* arg_ptr) {
  ...
  return X_STATUS_SUCCESS;
#else
  // POSIX: handle FIONBIO (non-blocking mode)
  if (cmd == 0x8004667E) {
    uint32_t value = *reinterpret_cast<uint32_t*>(arg_ptr);
    int flags = fcntl(native_handle_, F_GETFL, 0);
    if (flags < 0) return X_STATUS_UNSUCCESSFUL;
    flags = (value != 0) ? (flags | O_NONBLOCK) : (flags & ~O_NONBLOCK);
    return fcntl(native_handle_, F_SETFL, flags) < 0
...
int XSocket::WSAEventSelect(uint64_t socket_handle, uint64_t event_handle,
  ...
  return ::WSAEventSelect(socket_handle, reinterpret_cast<HANDLE>(event_handle),
                          flags);
#else
  // No equivalent on POSIX/iOS — return success silently.
  return 0;
#endif
}
