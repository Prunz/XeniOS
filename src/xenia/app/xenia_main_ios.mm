...
bool EmulatorAppIOS::OnInitialize() {
  // Configure AVAudioSession before any SDL or emulator audio initialisation.
  // Without this, iOS (especially iOS 26) will choose a default session
  // category that does not support multi-stream playback, causing crackling
  // and static when multiple guest audio clients are active.
  @autoreleasepool {
    ...
    [session setCategory:AVAudioSessionCategoryPlayback
                 ...
                 options:AVAudioSessionCategoryOptionMixWithOthers
                   error:&audio_session_error];
    if (audio_session_error) {
      XELOGW("iOS: AVAudioSession setCategory failed: {}",
             [[audio_session_error localizedDescription] UTF8String]);
      audio_session_error = nil;
    }
    [session setActive:YES error:&audio_session_error];
    if (audio_session_error) {
      XELOGW("iOS: AVAudioSession setActive failed: {}",
             [[audio_session_error localizedDescription] UTF8String]);
    }
  }
