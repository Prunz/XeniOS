...
#if XE_PLATFORM_IOS
// On iOS all audio must go through the shared software mixer to avoid
// CoreAudio rejecting a second SDL device open. The media player gets its
// own dedicated reserved slot so it never displaces a guest client.
auto* sdl_system =
dynamic_cast<xe::apu::sdl::SDLAudioSystem*>(audio_system_);
if (sdl_system) {
AudioDriver* raw_driver = nullptr;
X_STATUS status = sdl_system->CreateDriver(
xe::apu::sdl::SDLAudioSystem::kMediaPlayerMixSlot,
driver_semaphore_.get(), &raw_driver);
if (XSUCCEEDED(status) && raw_driver) {
driver_ = std::unique_ptr<AudioDriver>(raw_driver);
// Tell the mixer the actual channel count so it handles stereo
// FFmpeg output correctly instead of assuming 6-channel.
